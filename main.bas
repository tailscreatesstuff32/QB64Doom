...
DIM SHARED sprite_caco(8) AS LONG
...
DIM SHARED fake_walls(500) AS wall_data
DIM SHARED fake_walls0(100) AS wall_data
DIM SHARED fake_walls1(100) AS wall_data

DIM SHARED true AS LONG: true = 1

DIM SHARED false AS LONG: false = 0
...
SUB main
    ...
    sprite_caco(0) = _LOADIMAGE("sprites\caco\0.png", 32)
    sprite_caco(1) = _LOADIMAGE("sprites\caco\1.png", 32)
    sprite_caco(2) = _LOADIMAGE("sprites\caco\2.png", 32)
    sprite_caco(3) = _LOADIMAGE("sprites\caco\3.png", 32)
    sprite_caco(4) = _LOADIMAGE("sprites\caco\4.png", 32)
    sprite_caco(5) = _LOADIMAGE("sprites\caco\5.png", 32)
    sprite_caco(6) = _LOADIMAGE("sprites\caco\6.png", 32)
    sprite_caco(7) = _LOADIMAGE("sprites\caco\7.png", 32)
    sprite_caco(8) = _LOADIMAGE("sprites\caco\8.png", 32)
    ...
    sprite_objects temp_self(2), sprite_caco(0), false, 7, 4, -0.2, 0.74
    ...
    DO WHILE _KEYHIT <> 27
        ...
        sprite_object_locate temp_self(0), spr_object(0), my_player, walls(), buffer1D(300 + 0)
        sprite_object_locate temp_self(1), spr_object(1), my_player, walls(), buffer1D(300 + 1)
        sprite_object_locate temp_self(2), spr_object(2), my_player, walls(), buffer1D(300 + 2)
        sprite_object_locate temp_self(3), spr_object(3), my_player, walls(), buffer1D(300 + 3)
        sprite_object_locate temp_self(4), spr_object(4), my_player, walls(), buffer1D(300 + 4)


        'buffer1D(NUM_RAYS + 1).distance = spr_object(0).dist_sprite
        'buffer1D(NUM_RAYS + 2).distance = spr_object(1).dist_sprite
        'buffer1D(NUM_RAYS + 3).distance = spr_object(2).dist_sprite
        'buffer1D(NUM_RAYS + 4).distance = spr_object(3).dist_sprite
        'buffer1D(NUM_RAYS + 5).distance = spr_object(4).dist_sprite
    ...
    FOR i = 0 TO UBOUND(sprite_caco) - 1
        _FREEIMAGE sprite_caco(i)
...
SUB ray_casting (plyer AS player, tex() AS LONG) 'function?
    ...
    FOR ray = 0 TO NUM_RAYS - 1
        ...
        buffer1D(ray).distance = depth
        buffer1D(ray).posX = ray * SCALE
        buffer1D(ray).posY = INT(HALF_HEIGHT - proj_height / 2)
        buffer1D(ray).scalex = SCALE
        buffer1D(ray).scaley = proj_height
        buffer1D(ray).offx = offset * TEXTURE_SCALE
        buffer1D(ray).offy = 0
        buffer1D(ray).offwdth = TEXTURE_SCALE
        buffer1D(ray).offhght = TEXTURE_HEIGHT
        buffer1D(ray).tex = tex(texture)
        buffer1D(ray).show = 1
...
SUB world (world_objects() AS buffer_1D) ', wall_objects AS wall_data)


    'buffer1D.distance - perp walls distance
    'world_objects().dist_sprite - sprite distance
    ...
    FOR objects = 0 TO UBOUND(world_objects) '- 1
    ...
    '    IF world_objects(objects).show = 1 THEN                                                                                                                                                                                                                                      s
...
SUB DESC_SORT (obj() AS buffer_1D)





    'sort sprite drawing order
    FOR i = 1 TO UBOUND(obj)
        FOR j = 0 TO UBOUND(obj) - i


            IF obj(j).distance < obj(j + 1).distance THEN
    ...
    'FOR i = 1 TO UBOUND(buffer1D)
    '    FOR j = 0 TO UBOUND(buffer1D) - i


    '        IF buffer1D(j).distance < buffer1D(j + 1).distance THEN

    '            SWAP buffer1D(j), buffer1D(j + 1)
...
SUB sprite_object_locate (temp_slf AS sprite_data, self AS sprite_data, plyr AS player, walls() AS wall_data, buffer AS buffer_1D)
    DIM gamma AS SINGLE
    DIM theta AS SINGLE
    ...
    FOR i = 0 TO 100 - 1
        fake_walls0(i) = walls(0)
        fake_walls1(i) = walls(299)
    ...
    FOR i = 0 TO 500 - 1
        IF i < 100 THEN
            fake_walls(i) = fake_walls0(i)
        ELSE IF i >= 100 AND i < 400 THEN
                fake_walls(i) = walls(i - 100)
            ELSE IF i >= 400 AND i < 500 THEN
                    fake_walls(i) = fake_walls1(i - 400)
                END IF
            END IF
        END IF
    NEXT i
    ...
    ' _TITLE StrNum$(fake_ray)
    IF (fake_ray >= 0) AND fake_ray <= NUM_RAYS - 1 + 2 * FAKE_RAYS AND distance_to_sprite < fake_walls(cur_ray).dpth THEN
        ...
        IF temp_slf.static_spr = false THEN
            ...
            END IF
            theta = 360 - INT(radstodegs(theta))

            '   _TITLE (STR$(INT(theta)))
            ...
            '    temp_slf.tex = sprite_caco(0)

            '    ' exit for
            'END IF
            IF theta > 135 AND theta < 180 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(3)

                '         exit for
            END IF


            IF theta > 90 AND theta < 135 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(2)

                '         exit for
            END IF

            IF theta > 45 AND theta < 90 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(1)

                '         exit for
            END IF

            IF theta > 0 AND theta < 45 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(0)
            ...
            IF theta > 315 AND theta < 360 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(7)

                '         exit for
            END IF

            IF theta > 270 AND theta < 315 THEN
                '    self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(6)
            ...
            IF theta > 225 AND theta < 270 THEN
                'self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(5)

                'exit for
            END IF

            IF theta > 180 AND theta < 225 THEN
                '  self.object = self.sprite_positions[angles]
                temp_slf.tex = sprite_caco(4)
                '
                '   exit for
            END IF
        ...
        buffer.posX = (current_ray * SCALE) - half_proj_height
        buffer.posY = HALF_HEIGHT - half_proj_height + shift
        buffer.show = 1
        buffer.scalex = proj_height
        buffer.scaley = proj_height
        buffer.distance = distance_to_sprite
        buffer.tex = temp_slf.tex
        buffer.offwdth = _WIDTH(temp_slf.tex)
        buffer.offhght = _HEIGHT(temp_slf.tex)
        buffer.offx = 0
        buffer.offy = 0
